...
class Api extends \Modulos\Vpbasicos\Controllers\Apibase
{
    ...
    public function registrarCuenta($clavePublica){
        $respuesta=['codigo'=>400,'tipo'=>'Error','mensaje'=>'Error indeterminado'];
        log_message('debug','En registrar cuenta');
        $res=$this->autentifica();
        if ($res['tipo']=='exito'){
            ...
            if (!preg_match('/^[0-9A-Z]{55}$/',$clavePublica)){
                ...
                if ($resUsuario!='OK'){
                    ...
                    if (!is_null($cuenta)&&($cuenta->clave!=$clavePublica)){
                        log_message('debug','nueva cuenta');
                        $cuenta->actualizaEstado();
                        if ($cuenta->creada==1){
                    ...
                    if (is_null($cuentaIndicada)){
                        ...
                        try{
                            ...
                            log_message('debug','Añadida');
                        }
                        catch(\Exception $e){};
                    }
                    else{
                        $cuentaIndicada->actualizaEstado();
                        log_message('debug','Existe. Autorizada='.$cuentaIndicada->autorizada);
                    }
                    log_message('debug','registro..');
                    
                    $usuario->registraCuenta($cuentaIndicada);
                    if ($cuentaIndicada->creada==0){
                        log_message('debug','No Existe. La creo');
                        $cuentaIndicada->create();
                    }
                    else{
                        log_message('debug','Existe. Aseguro XLM');
    ...
    public function autorizarCuenta($clavePublica){
        log_message('debug','En autorizar cuenta');
         $respuesta=['codigo'=>400,'tipo'=>'Error','mensaje'=>'Error indeterminado'];
        $res=$this->autentifica();
        if ($res['tipo']=='exito'){
            //Si viene con 'G', se la quito: GBOAKWTFL7IL4EORXRAMIVNCCK3ECZOKIJS7Y7WO53FHPAD44PGSRX3P
    ...
    public function comercios(){
        ...
        if ($res['tipo']=='exito'){
            ...
            $idClase=$this->esBeneficiario?($this->beneficiario->clase??0):0;
            $clase=model('Modulos\Pagina\Models\Cls_clases')->find($idClase);
            $textoClase=\Modulos\Pagina\TODOS_LOS_COMERCIOS;
            if (!is_null($clase)){
                $textoClase=$clase['clase'];
            }
            $lista=model('Modulos\Pagina\Models\Cls_comercios')->deClase($idClase);
            $respuesta=['clase'=>$textoClase,'listado'=>[]];

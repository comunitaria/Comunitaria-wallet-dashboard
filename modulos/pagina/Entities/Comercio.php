...
class Comercio extends UsuarioILLA
{
    public function ficheroLogo(){
        if ($this->logo)
            return base_url('public/assets/imagenes/logos').(file_exists(ROOTPATH . 'public/assets/imagenes/logos/logo'.$this->id.'.png')? '/logo'.$this->id.'.png?'.strtotime('now') : '/logo0.png');
        else
            return base_url('public/assets/imagenes/logos').'/logo0.png';
    }
    ...
    public function pago($mes,$ano,$importe,$factura,$notas){
        ...
        if ($db->query("INSERT INTO pagos (usuario,euros,de_mes,de_ano,factura,notas) VALUES (".$this->id.",".floatval($importe).",".intval($mes+1).",".intval($ano).",'".$factura."','".$notas."')")){
            ...
            log_message('debug','registrando pago '.$this->pagadoILLA);
            try{

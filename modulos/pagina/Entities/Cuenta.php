...
class Cuenta extends Entity
{
    public function actualizaEstado(){
        ...
        if (!$balances['existe']){
            ...
            log_message('debug','actualizada='.$this->autorizada);
            $this->creada=1;
        }
    ...
    public function create(){
        log_message('debug','creando '.$this->clave);
        $st=new Stellar();
    ...
    public function autorizate(){
        ...
        log_message('debug','se supone que autorizada');
        $this->actualizaEstado();
        return $ok;
    }
    ...
    public function aseguraXLM(){
        ...
        if ($this->balanceXLM<getenv('moneda.XLM.minimo')){
            log_message('debug','Balance NOK='.$this->balanceXLM);
            $st=new Stellar();
            ...
            log_message('debug','Balance OK='.$this->balanceXLM);
            return true;
        }
    }

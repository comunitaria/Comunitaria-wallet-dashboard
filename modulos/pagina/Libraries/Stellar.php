...
class Stellar
{
    ...
    private function crearCuentaReal($de,$a,$cantidad,$privadaDe){
        log_message('debug','Entro a crear cuenta');
           
        $cuentaDe = $this->sdk->requestAccount('G'.$de);
        $pagadorPar = KeyPair::fromPrivateKey(StrKey::decodeSeed('S'.$privadaDe));
        log_message('debug','Parece que hay pagador');
        try{
            log_message('debug','Entro a crear cuenta '.'G'.$a);
            log_message('debug','Entro a crear cuenta con '.number_format($cantidad,7));
    ...
    public function transfiereCripto($de,$a,$cantidad,$privadaDe=''){
        ...
        if (($de=='emisora')||($de=='distribuidora')){
            return $this->transfiereCriptoReal(getenv('moneda.'.$de.'.publica'),$a,number_format($cantidad,7),getenv('moneda.'.$de.'.privada'));
        }
        else{
    ...
    private function transfiereCriptoReal($de,$a,$cantidad,$privadaDe){
        $respuesta=['exito'=>false,'mensaje'=>'Transferencia imposible'];
        $cuentaDe = $this->sdk->requestAccount('G'.$de);
        $pagadorPar = KeyPair::fromPrivateKey(StrKey::decodeSeed('S'.$privadaDe));
        try{
    ...
    private function motivoErrorStellar($dice){
        try{
            switch($dice->getStatus()){
                case '404':
                    log_message('debug',print_r($dice,true));

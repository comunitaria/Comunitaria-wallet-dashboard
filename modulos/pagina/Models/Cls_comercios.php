...
class Cls_comercios extends Model
{
    ...
    public function deClase($idClase){
        if ($idClase==0){
            ...
            if (count($listaIds)>0){
                log_message('debug',print_r($listaIds,true));
    ...
    public function deClases($clases=[0]){
        ...
        foreach($res as $unIdComercio){
          ...
          $lista[]=[
            ...
            'Logo'=> ($unComercio->logo=='1')?(base_url('public/assets/imagenes/logos').(file_exists(ROOTPATH . 'public/assets/imagenes/logos/logo'.$unComercio->id.'.png')? '/logo'.$unComercio->id.'.png?'.strtotime('now') : '/logo0.png')):
                                             (base_url('public/assets/imagenes/logos').'/logo0.png'),
